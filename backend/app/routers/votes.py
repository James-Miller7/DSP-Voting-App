...
@router.post("/candidates/{candidate_id}/votes")
async def submit_vote(candidate_id: str, vote: VoteCreate, user = Depends(get_current_user), supabase = Depends(get_supabase)):
  if user["role"] == "admin":
    raise HTTPException(status_code=403, detail="Admins can not vote")
...
async def reset_votes(supabase = Depends(get_supabase), user = Depends(is_admin_user)):
  await supabase.table("votes").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
...
async def get_non_voters(candidate_id: str, supabase = Depends(get_supabase), user = Depends(is_admin_user)):
  members_response = await supabase.table("profiles").select("id", "full_name").eq("role", "member").execute()
